...
Code used for manuscript assembly, source-data indexing, clinical sensitivity modeling, and reviewer-facing reproducibility checks is available at \url{https://github.com/Luciky-Leo/Luciky-Leo-mregdc-tls-ov} and archived at Zenodo under DOI \url{https://doi.org/10.5281/zenodo.21184531}. Figure-level source-data files are provided in the repository under \texttt{source\_data/} and are mapped to current manuscript figures in \texttt{source\_data/SOURCE\_DATA\_INDEX.csv}. Additional upstream analysis scripts used to derive the single-cell, spatial, immune-deconvolution, perturbation, prognostic-model, and docking source tables are available from the corresponding authors on reasonable request.
