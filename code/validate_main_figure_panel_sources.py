...
This script is intentionally conservative. It checks that each mapped
main-figure panel has reviewer-accessible source files and records compact
...
def main() -> int:
    ...
    with mapping.open("r", encoding="utf-8-sig", newline="") as handle:
        for record in csv.DictReader(handle):
            ...
            script_status = "exists" if script.exists() else "missing"
                ...
                rows.append(
                    {
                        ...
                        "panel": record["panel"],
                        "script": str(script.relative_to(root)) if script.is_absolute() else record["script"],
