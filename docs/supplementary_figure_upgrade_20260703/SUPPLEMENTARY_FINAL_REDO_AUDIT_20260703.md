...
Final visual iteration:

- S4 was further tightened after visual QC: the top multi-sample spatial gallery was resized to a 180 mm x 140 mm canvas, co-high spots were enlarged, dataset labels were centered, and each spatial thumbnail was rendered in an equal-sized normalized map slot so the multi-sample maps align visually.
- S10 was further refined after visual QC: a pale tissue-density background was added under the TLS-field grid, field/co-high markers were rescaled, and the embedded SI page was re-rendered for readability.
- S1, S2, and S7 were updated with white-background direct labels and connector lines to avoid point-label overlap.
- S8 was updated with shortened disease/marker labels and a log10(count+1) IF raw-count heatmap, replacing the crowded grouped-bar legend.
- S13 feature labels were shortened to avoid x-axis label overlap in the patient-level heatmap panels.
- Final QC previews were generated at `qc_reports\SUP_CNS_v2_final_contact_sheet.png`, `qc_reports\SI_page_04_render.png`, and `qc_reports\SI_page_10_render.png`.
...
After the final S4/S10 spatial-figure iteration, the Supplementary Information PDF and upload packages were rebuilt again from the updated figure PDFs.
